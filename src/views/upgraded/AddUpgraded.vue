    ...
    <div class="ms-doc">
      ...
      <article>
        <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="150px" class="demo-ruleForm">
          <div v-for=" (firmwareInfo, index) in ruleForm.Firmwares" :key="index">
            <el-form-item :label="'选择固件'+firmwareInfo.Index" :prop="'Firmwares.' + index + '.FirmwareId'">
              <el-select v-model="firmwareInfo.FirmwareId" filterable placeholder="请选择">
                <el-option
            ...
            <el-form-item label="升级模式" :prop="'Firmwares.' + index + '.UpgradeMode'">
              <el-select v-model="firmwareInfo.UpgradeMode" placeholder="请选择">
            ...
            <el-form-item label="升级顺序编号：" :prop="'Firmwares.' + index + '.Index'">
              <el-input v-model.number="firmwareInfo.Index" placeholder="请输入数字"></el-input>
            </el-form-item>
          </div>
          <el-form-item>
    ...
    methods: {
      ...
      listView: function () {
        ...
        this.$http({
          ...
          console.log("固件列表：",res.data.Results)
      ...
      submitForm(formName) {
        ...
        options = this.options,
          ...
          console.log("options",_Firmwares)
        _Firmwares.forEach((Firmwares) => {
          for( var i = 0; i<options.length;i++){
            if(options[i].FirmwareId===Firmwares.FirmwareId){
                _data.push({
                ...
                Name: options[i].Name,
                FileSize: options[i].FileSize,
                FileUrl: options[i].FileUrl,
                FileMd5: options[i].FileMd5,
                FirmwareId: options[i].FirmwareId,
      ...
      add_err(error) {
        ...
        if (error.response) {
          ...
          if (error.response.data.ErrCode == 40000101) {
            self.$message.error('新建OTA版本失败');
          } else if(error.response.data.ErrCode == 40000101){
            self.$message.error('可升级设备列表不能为空');
          }else{
            self.$message.error('新建OTA版本失败');
            return false;
          }
